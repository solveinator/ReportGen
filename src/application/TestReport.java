...
//	}
	...
	protected void makeQueries(String startTimeStamp, String endTimeStamp) {
		String query = "SELECT LogID, ProductID, DonorID, LogDate, DonorRef, DonorName, "
				...
				+ "And NOT EXISTS "
					+ "(SELECT * "
					+ "FROM NFBSData.dbo.uqryReceiptDetailwoDates as B "
					...
					+ "AND (((DonorRef >= 'PA1703' AND DonorRef <= 'PA974') "
						...
						+ "OR (B.LogID IN "
							+ "(SELECT BB.LogID "
							+ "FROM NFBSData.dbo.uqryReceipts_Receipts as BB "
							+ "WHERE BB.\"Donation Type\" Like '*Purchase%'))) "
					+ "AND (A.LogID = B.LogID AND A.ProductRef = B.ProductRef) "
					+ ") "
				+ "And NOT EXISTS "
					+ "(SELECT * "
					+ "FROM NFBSData.dbo.uqryReceiptDetailwoDates as C "
				...
				+ "AND NOT EXISTS " 
					+"(SELECT * "
					+ "FROM NFBSData.dbo.uqryReceiptDetailwoDates as E "
				...
				+ "AND NOT EXISTS "
					+ "(SELECT * " 
					+ "FROM NFBSData.dbo.uqryReceiptDetailwoDates as J "
					+ "WHERE (J.Category = 'GOVERNMENT') "
					+ "AND (A.LogID = J.LogID AND A.ProductRef = J.ProductRef) "
					+ ") "
				+ "AND NOT EXISTS "
					+ "(SELECT * "
					+ "FROM NFBSData.dbo.uqryReceiptDetailwoDates as K "
					+ "WHERE K.LogID NOT IN "
						+ "(SELECT L.LogID "
